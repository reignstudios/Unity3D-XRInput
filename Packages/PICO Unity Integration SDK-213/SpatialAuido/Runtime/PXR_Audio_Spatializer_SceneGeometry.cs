...
public class PXR_Audio_Spatializer_SceneGeometry : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        {
            meshConfig = new MeshConfig(true, Material,
                Matrix4x4.Scale(new Vector3(1, 1, -1)) * transform.localToWorldMatrix);
            if (geometryId >= 0)
    ...
    private void Update()
    {
        ...
        {
            meshConfig.SetTransformMatrix4x4(Matrix4x4.Scale(new Vector3(1, 1, -1)) * transform.localToWorldMatrix);
    ...
    private static Mesh CombineMeshes(List<MeshFilter> meshFilterList, Transform rootTransform)
    {
        ...
        foreach (var meshFilter in meshFilterList)
        {
            ...
            var toWorld = Matrix4x4.Scale(new Vector3(1, 1, -1)) * rootTransform.worldToLocalMatrix *
                          meshFilter.transform.localToWorldMatrix;
            for (int i = vertexOffset; i < vertexSegmentEnd; ++i)
            {
                combinedVertices[i] = toWorld.MultiplyPoint(combinedVertices[i]);
            }
        ...
        combinedMesh.triangles = combinedIndices.Reverse().ToArray();
    ...
    public PXR_Audio.Spatializer.Result SubmitMeshToContext()
    {
        ...
        meshConfig = new MeshConfig(enabled, Material,
            Matrix4x4.Scale(new Vector3(1, 1, -1)) * transform.localToWorldMatrix);
